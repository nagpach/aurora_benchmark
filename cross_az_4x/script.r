...
config <- "Amazon RDS (MySQL Multi-AZ, Aurora; Cross-AZ) vs. Google Cloud SQL Second Generation\nsysbench 0.5, 100 x 20M rows (2B rows total), 30 minutes per step"
steps <- c(1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024)
time_per_step <- 1800
output_path <- "~/src/demos/aurora_benchmark/cross_az_4x/"
...
results <- data.frame(
  ...
  file = c(
    "~/src/demos/aurora_benchmark/cross_az_4x/01_baseline_all.csv"
  ),
  name = c(
    "Amazon RDS Aurora"
  ),
